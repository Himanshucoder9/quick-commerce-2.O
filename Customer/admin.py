...
from Auth.admin import ReadOnlyAdmin
...
class ShippingAddressAdmin(ReadOnlyAdmin, ImportExportModelAdmin, admin.ModelAdmin):
    ...
    list_display = ['id', 'customer', 'customer_name', 'customer_phone', 'address_type', 'building_name', 'floor',
                    'landmark']
...
class FavoriteAdmin(ReadOnlyAdmin, ImportExportModelAdmin, admin.ModelAdmin):
    fieldsets = (
        ('Info', {
            'fields': (
                'customer', 'product'),
...
@admin.register(Cart)
class CartAdmin(ReadOnlyAdmin, ImportExportModelAdmin, admin.ModelAdmin):
    fieldsets = (
        ('Info', {
            'fields': ('customer',),
        }),
...
class CartItemAdmin(ReadOnlyAdmin, ImportExportModelAdmin, admin.ModelAdmin):
    fieldsets = (
        ('Info', {
            'fields': (
...
class OrderAdmin(ImportExportModelAdmin, admin.ModelAdmin):
    ...
    def _shipping_address(self, obj):
        if obj.shipping_address:
            link = reverse("admin:Customer_shippingaddress_change", args=[obj.shipping_address.pk])
    ...
    list_display = ['id', 'order_number', 'customer', 'total_amount', 'order_status', 'payment_method',
                    '_shipping_address',
                    'created_at']
...
class OrderItemAdmin(ReadOnlyAdmin, ImportExportModelAdmin, admin.ModelAdmin):
    fieldsets = (
        ('Info', {
            'fields': (
    ...
    list_display = ['id', 'order', 'warehouse', 'product', 'quantity', 'item_price', 'created_at', ]
...
class PaymentAdmin(ReadOnlyAdmin, ImportExportModelAdmin, admin.ModelAdmin):
    fieldsets = (
        ('Payment Details', {
            'fields': (
                'customer', 'order', 'payment_method', 'razorpay_order_id', 'razorpay_payment_id',
    ...
    list_display = ['id', 'customer', 'order', 'razorpay_payment_id', 'amount', '_payment_status', 'payment_method',
                    'payment_date']
