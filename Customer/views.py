...
class FavoriteViewSet(ModelViewSet):
    ...
    serializer_class = FullFavoriteSerializer
    permission_classes = [IsAuthenticated]
    ...
    def create(self, request, *args, **kwargs):
        ...
        if Favorite.objects.filter(customer=request.user, product_id=product_id).exists():
        ...
        for item_data in cart_items_data:
            ...
            if product.quantity_in_stock < quantity:
                return Response({"error": f"Not enough stock available for product ID {product_id}",
                                 "available_quantity": product.quantity_in_stock}, status=status.HTTP_400_BAD_REQUEST)
...
class CartItemDeleteAPIView(APIView):
    ...
    def delete(self, request, pk):
        ...
        cart_item.delete()
        return Response({"message": "Product successfully deleted from cart"}, status=status.HTTP_204_NO_CONTENT)
        ...
        with transaction.atomic():
            ...
            for item_data in items:
                ...
                OrderItem.objects.create(order=order, warehouse=product.warehouse, product=product,
                                         quantity=item_data.get('quantity'), item_price=item_price)
